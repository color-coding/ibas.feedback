...
namespace feedback {
    export namespace app {
        ...
        export class SuggestionEditApp extends ibas.BOEditApplication<ISuggestionEditView, bo.Suggestion> {
            ...
            /** 应用名称 */
            static APPLICATION_NAME: string = "feedback_app_suggestion_edit";
            /** 业务对象编码 */
            ...
            run(): void {
                let that: this = this;
                if (ibas.objects.instanceOf(arguments[0], bo.Suggestion)) {
                    let data: bo.Suggestion = arguments[0];
                    // 新对象直接编辑
                    if (data.isNew) {
                        that.editData = data;
                        that.show();
                    ...
                    if (!ibas.objects.isNull(criteria) && criteria.conditions.length > 0) {
                        ...
                        boRepository.fetchSuggestion({
                            ...
                            onCompleted(opRslt: ibas.IOperationResult<bo.Suggestion>): void {
                                let data: bo.Suggestion;
                                if (opRslt.resultCode === 0) {
                                    data = opRslt.resultObjects.firstOrDefault();
                                }
                                if (ibas.objects.instanceOf(data, bo.Suggestion)) {
                                    // 查询到了有效数据
                                    that.editData = data;
                                    that.show();
                                } else {
        ...
        /** 视图-建议 */
        export interface ISuggestionEditView extends ibas.IBOEditView {
            /** 显示数据 */
